...
fn a_press_is_instant_and_costs_the_tower_nothing() {
    ...
    // `PRESS_TICKS = 0` (§19): a press was twelve ticks of the one production
    // slot, so a bound solver now runs beside a full brewing loop with no
    // contention. The claim here is that the answer is in the same block as the
    // word — no `meditate` between.
    let game = Game::start();
    game.does("attend lens", "/tower/lens")
        .does("probe", "aligned");
}
...
fn there_is_no_lens_to_look_through_elsewhere() {
    ...
    game.does("attend laboratory", "/tower/laboratory")
        ...
        // which is what scopes a verb to the domain that declares it.
...
fn a_socket_that_does_not_exist_is_answered_with_ones_that_do() {
    ...
    // `fifth` never reaches the socket check: the parser resolves nouns before
    // the verb sees them, so an unknown word comes back as numbered suggestions
    // (`»`) drawn from what the tower has.
    let game = Game::start();
    game.does("attend lens", "/tower/lens")
        .does("probe", "four sigils hold it shut")
...
fn survey_answers_two_different_questions_about_one_ward() {
    ...
    // spell reads which way each one moved. It was three — a socket also
    // reported `settled`, `loose` and a count of what it had left to try (§19).
    let game = Game::start();
    game.does("attend lens", "/tower/lens")
        .does("probe", "four sigils hold it shut")
...
fn a_socket_never_says_whether_it_is_right() {
    ...
    // The rule the whole domain rests on, from the player's side: no codemaker
    // answers a question about one position, so whether a socket is settled is
    // the player's own bookkeeping. Three words leaked it — `settled`, `loose`
    // and `untried` — the last silently, by falling to nought on that fact.
    let game = Game::start();
    game.does("attend lens", "/tower/lens")
        .does("probe", "four sigils hold it shut")
...
fn the_board_is_drawn_whenever_a_reading_is_open() {
    ...
    // The sheet names its own columns and sigils, and it was ten cells before it
    // was thirty-nine (§19): nothing said which column was which socket, and
    // nothing said `♦` was `pewter`. Both name tables come from the sim through
    // `Ward::view` — they are content, and `orbs-render` may not depend on
    // `orbs-sim`.
    let game = Game::start();
    game.does("attend lens", "/tower/lens")
        .does("probe", "four sigils hold it shut")
