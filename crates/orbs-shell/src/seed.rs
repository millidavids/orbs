...
//! frontend chooses a number once, the sim is built from it, and the save carries
//! it from then on. The POST card prints it, so a tester's report says which world
//! they were in.
//!
//! # Two readers, and which one a caller wants is the whole question
//!
//! - [`seed`] — `ORBS_SEED`, or [`SEED`]. **Every instrument**: `ORBS_DUMP`, the
//!   `screens` example, `scripts/dumps.sh`, the play harness, and every See-it
//!   line. A dump that drew a different world each run could not be diffed.
//! - [`new_game_seed`] — `ORBS_SEED`, or a seed of the game's own. **A game a
//!   player starts**: the first tower on a machine with no save, and a new game
//!   from the menu. Before `0.15.5` every player's tower was seed `0x0B5` — the
...
///
/// **Short enough to read off the POST card and type back**, which is what a
/// tester does with a world worth reporting — `ORBS_SEED=482913`. A million
/// towers is variety no player will exhaust, and nothing about determinism needs
/// the whole of `u64`.
...
/// The seed a **new game** is built from: `ORBS_SEED` if it names a number, else
/// a seed of its own from the wall clock.
///
/// **A screenshot is an instrument**, so with `ORBS_CAPTURE` set this is [`SEED`]
/// — the capture's boot card prints the seed, and a reproducible screenshot is
/// the reason the switch exists. An `ORBS_SEED` that names no number is treated
/// as unset: the game still gets a seed of its own rather than the default.
#[must_use]
...
///
/// **Pure, so it is testable without the clock.** The nanoseconds are folded
/// through splitmix64 so that two games begun a moment apart are not neighbouring
/// seeds, and the result is taken below [`SEEDS`] so it reads in six digits.
#[must_use]
...
mod tests {
    ...
    /// **Six digits, and spread across them** — a thousand moments a nanosecond
    /// apart land on nearly a thousand different seeds, all below the bound.
    #[test]
    ...
    /// **The high half of the moment counts too**, so a clock far from 1970 does
    /// not fold two different moments onto one seed through the low bits alone.
    #[test]
