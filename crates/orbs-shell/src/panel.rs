...
#[must_use]
pub fn split(area: Rect, instruments: &[Instrument]) -> Split {
    let nothing = Split {
        area: Rect::EMPTY,
        along: Along::Top,
        rest: area,
...
const fn stand_in(state: State) -> Meter {
    // The unit is never read here — this feeds a *bar*, which is a ratio — but
    // it has to be something, and `Ticks` is what a meterless instrument would
    // be counting if it had one.
    match state {
        State::Ready => Meter {
            done: 1,
            total: 1,
            unit: orbs_sim::tower::Unit::Ticks,
        },
        _ => Meter {
            done: 0,
            total: 1,
...
mod tests {
    ...
    // The sim-to-colour test that used to sit here is in
    // `crate::render::tinting`. It reaches `render::{palette, tint}`, which
    // resolve a `Style` to a `bevy::Color` and are the Bevy frontend's alone
    // (rule 2) — so it cannot travel with this painter.
