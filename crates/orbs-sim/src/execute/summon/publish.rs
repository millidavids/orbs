...
///
/// Takes the circle rather than reading `Cwd`, which avoids the lens's recorded
/// bug by construction: whoever calls this has already found the circle, and a
/// publisher that looked again could look in the wrong room.
///
...
/// `taming` limns up to 216 times a beast — so republishing everything despawned
/// and re-raised four readings each time to say three of them again with new
/// ids, the churn `call` was changed to stop.
...
fn publish_glyph(world: &mut World, room: Entity, at: Entity, glyph: Glyph) {
    ...
    // What it is limned with, and nothing else — the lens's socket carrying its
    // sigil. Never whether it is right: a verdict on a glyph would be the orb
    // solving the circle (§19). A dark glyph says nothing, since at a lesser
    // circle the outer two are not part of it, so a spell asking what they are
    // limned with hears nothing rather than the opening.
    let limned = world
        .get::<Beast>(at)
