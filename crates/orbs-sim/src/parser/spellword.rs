...
///
/// # Two shapes, and no operators
///
/// §6's whole posture is that a player types what they mean. `if the dispensary
/// has sage` and `if the mortar is idle` are sentences; `if count(sage) > 0` is
/// a different program wearing the game's clothes. There are no comparisons, no
/// booleans, no `and`, and no nesting of conditions — each of those is a thing
/// to learn that the tower never taught.
...
impl Condition {
    ...
    ///
    /// Both shapes have one, and the runner needs it by name to say *which*
    /// place a question could not find — §8.1: the culprit is never anonymous.
    #[must_use]
    pub fn place(&self) -> &str {
        match self {
            Self::Has { place, .. } | Self::Is { place, .. } => place,
        }
    }
}
...
/// Write a question back out, as the orb writes it down.
///
/// The counterpart of [`condition`], and what lets an `if` be canonicalised at
/// save like every other line: the orb reads `if the mortar is empty`, resolves
/// `mortar` against the room the way a command's argument is resolved, and
/// writes `if mortar_and_pestle is empty`.
///
/// **Without this an `if` was the one line the orb did not write down**, and the
/// consequence was silent: `holds` compares a place name *exactly*, so
/// `if mortar is empty` never found the mortar, answered no every time, and took
/// the `else` for ever. It looked exactly like the condition being inverted.
#[must_use]
...
///
/// Returns `None` for anything it cannot make sense of, and the caller keeps the
/// line flagged rather than guessing — the same rule the rest of the spell
/// language follows, because canonicalisation runs at save and a guess would be
/// written into the file.
#[must_use]
pub fn condition(tail: &str) -> Option<Condition> {
    let words: Vec<&str> = tail
        .split_whitespace()
        .filter(|word| !super::normalise::is_filler(word))
        .collect();

    if let Some(at) = words.iter().position(|word| *word == "has") {
        let (place, thing) = (words.get(..at)?, words.get(at + 1..)?);
        if place.is_empty() || thing.is_empty() {
            return None;
        }
        return Some(Condition::Has {
            place: place.join(" "),
            thing: thing.join(" "),
        });
    }

    let at = words.iter().position(|word| *word == "is")?;
    let (place, rest) = (words.get(..at)?, words.get(at + 1..)?);
    let state = match rest.first()? {
        &"idle" | &"free" | &"still" => State::Idle,
        &"working" | &"busy" | &"running" => State::Working,
        &"empty" | &"bare" => State::Empty,
        _ => return None,
    };
    if place.is_empty() {
        return None;
