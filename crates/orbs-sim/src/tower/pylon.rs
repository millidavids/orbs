...
//!
//! The Tower of Hanoi, because §10's *"command pressure at 1 Hz"* is a reflex
//! mechanic unless something makes it a decision: no clock, every ward on
//! screen, outcome following what the player chooses (§10.1).
//!
//! Raw arcane energy wells up in the [`WELLSPRING`]; the wizard draws it
//! through the [`CONDUIT`] a ward at a time and assembles it into the
//! [`BARRIER`] — a magical defence, not masonry (§19).
//!
//! A greater ward will not rest upon a lesser, so between any two stations
//! exactly one haul is legal unless both are empty. Choosing a *pair* has
//! chosen the move, and a spell needs only which way round it runs — what
//! [`POTENCY`] is for; `a_haul_between_two_stations_is_unique` proves it.
//!
//! A course is three to seven wards ([`LEAST`], [`MOST`]), from how far
//! integrity has slipped — see [`erosion`](super::erode). 127 hauls against
//! seven, so neglect is expensive without being ruinous (§11.5). The height
//! also decides which way the cycle runs, which is why [`ODD`] is a reading,
//! and [`height_for`](super::height_for) jitters it so it cannot be hard-coded.
...
///
/// A course is drawn up at the first and belongs at the last; the middle is the
/// spare. A `Course` addresses them by index, so nothing depends on the names.
...
/// The tallest: 127 hauls. Held against the painter's reservation by
/// `a_board_has_room_for_the_tallest_course`, since `orbs-render` may not
/// depend on this crate.
pub const MOST: usize = orbs_render::TALLEST;

/// How great the ward at the top of a station is. A counted reading, on each.
///
/// `potency`, not `heft`: arcane energy has magnitude and no mass (§19).
/// Published only while the station holds a ward — `spell::watch` reads `is
/// empty` as "no children", so an always-present `potency` would make every
/// station look occupied.
...
///
/// Registered unconditionally in `tower::scene` whether or not a course is
/// open: a spell compiles at cast, when none of them is true of anything.
#[must_use]
pub fn readings() -> Vec<&'static str> {
    vec![POTENCY, ODD, INTEGRITY]
}

/// Which station a name means, by leaf — a resolved `Place` argument arrives
/// as `/tower/sanctum/wellspring`.
#[must_use]
...
///
/// Not `Cwd`: [`erode`](super::erode) runs on a tick wherever the player is.
#[must_use]
...
/// Why a haul was not made. Each names something different to do about it
/// (§6).
...
/// A course of wards, mid-solve. Fields are private because only
/// [`haul`](Self::haul) can be trusted to keep every station descending.
...
impl Course {
    ...
    ///
    /// Clamped to `LEAST..=MOST`: the height comes from an integrity deficit
    /// and a jitter, and a course of nought is solved the moment it is drawn.
    #[must_use]
    ...
    ///
    /// Returns the magnitude that moved. No refusal changes the course.
    ///
    /// # Errors
    ///
    ...
    ///
    /// `None` only when both are empty or they are the same station. Nothing in
    /// the game calls it; the tests and the cyclic solver below do.
    #[must_use]
    ...
    /// Hauls spent are kept, so the finish record quotes a real number.
    ...
    /// What a frontend needs to draw this. `tally` is the already-written line
    /// under the floor rule; `orbs_render::Pylon::tally` says why.
    #[must_use]
    pub fn view(&self, integrity: u32, tally: String) -> orbs_render::Pylon {
        orbs_render::Pylon {
            stations: self.stations.clone(),
            tally,
            // The words, so the board can be typed from: `orbs-render` may not
            // depend on this crate, and an unlabelled column is counted along.
            names: STATIONS,
            height: self.height,
            hauls: self.hauls,
            integrity,
    ...
    /// Put one back. Checked rather than trusted: eight wards at one station
    /// would draw off a board that reserves seven rows.
    pub(crate) fn from_save(save: &crate::save::CourseSave) -> Option<Self> {
        ...
        // Refused, not repaired: clamping the count up to `LEAST` gave a
        // three-ward course holding two, so `solved()` was unreachable for the
        // session. `save.height` is the checksum — every ward stays in the
        // course for its whole life, so a disagreement means the file is wrong.
        ...
        // The invariant only `haul` keeps. A station that ascends is an illegal
        // position, and sorting it would invent a course nobody had.
        if stations
            .iter()
...
///
/// The algorithm the shipped `holding` spell writes, here so a test can prove
/// it terminates without the interpreter. Even and odd courses rotate opposite
/// ways round the stations. `dev_spells.toml` has the same three lines in the
/// spell language.
#[must_use]
...
mod tests {
    ...
    fn a_haul_between_two_stations_is_unique() {
        // Swept rather than argued: at most one direction of a pair is legal,
        // and exactly one unless both stations are empty.
    ...
    fn a_greater_ward_never_rests_upon_a_lesser() {
        // An invariant of the model, not of one call: whatever a solve does,
        // every station descends the whole way through.
    ...
    fn the_cyclic_solution_finishes_every_course_in_exactly_two_to_the_n_less_one() {
        // The cycle is optimal, so a high count means the pairs are in the
        // wrong order for that parity. A wrong cycle still solves.
        for height in LEAST..=MOST {
            let mut course = Course::new(height);
    ...
    fn the_parity_is_what_decides_the_cycle_and_getting_it_wrong_is_visible() {
        // Why `ODD` is published: under the wrong parity a course still
        // terminates, at the wrong station, and reports success.
        let mut course = Course::new(4);
        let wrong = cycle(5);
        for _ in 0..1024 {
    ...
    fn the_line_under_the_floor_rule_fits_the_board_it_is_drawn_on() {
        // This shipped truncated: `pylon_tally` and `pylon_spoken` were one key
        // and `centred` cut the longer one silently. `hauls` is unbounded, so
        // the check uses a number wider than any real solve.
    ...
    fn a_board_has_room_for_the_tallest_course() {
        // From this side because `orbs-render` cannot see `MOST`. Too few
        // reserved rows clips the bottom of a neglected tower's course.
    ...
    fn a_malformed_course_is_refused_rather_than_repaired() {
        // Each of these came back as a jammed course when the height was
        // clamped up to `LEAST`. Refusing raises no course, and muster works.
