...
///
/// **Reset per beast, never carried over**, so a spell always knows where it
/// starts. A circle left as the last beast held it would make every solver's
/// first moves depend on history the spell cannot read.
pub const OPENING: [Humour; 3] = [Humour::Yoke; 3];

/// How many calls a beast may take and still pay in full.
///
/// **Three, because the temper is visible.** A player who reads the table needs
/// one call — the one that holds — and PAR leaves room for two that check a
/// guess, which is how a person learns the six. Past it a hold pays three
/// quarters, never nothing: the lens's `yield_of`, and its reason, that the tick
/// cost of a search already does the real work.
pub const PAR: u32 = 3;

/// Whether `sense` is lit on `row` of a table over `senses`.
///
/// **The first sense is the row's highest bit**, so row nought is every sense
/// dark and the last row every sense lit — the order a truth table is written
/// in, and the order the board draws its columns.
#[must_use]
...
///
/// **One byte, a row a bit**, row nought lowest, and the number of senses the
/// rows are over — three for the whole circle, two for the lesser (see
