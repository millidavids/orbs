...
//! `tower::burn` and `tower::finish` build per tick. The whole catch-up is
//! ~120 ms at ~4 µs a step, so those four constructions are noise at this
//! entity count. Re-run it before believing otherwise.
