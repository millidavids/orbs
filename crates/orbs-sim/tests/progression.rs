...
//!
//! Driven through real runs rather than by calling the grant — only the number
//! a player earns counts, and the two hooks (`work::produce::transmute`, the
//! archive's branch of `land::finish`) are the likeliest to be miswired.
...
fn one_clarity_is_exactly_the_first_threshold() {
    ...
    // moving, or the hook firing twice all show up here. Walked rather than
    // summed, because this is the path a player takes before automation.
    let mut sim = laboratory();
...
fn the_archive_earns_too() {
    ...
    //
    // Solved rather than merely opened: `divine` opens the stacks and reaching
    // the way out is what earns, so this walks one by the same Trémaux rule a
    // player writes as a spell.
    let mut sim = Sim::new(1);
...
fn choose(sim: &Sim) -> Option<&'static str> {
    ...
    // Five rungs; the last two are what make it terminate. With four, a
    // junction where two ways read alike sends a fixed compass order back the
    // way it came and cycles. `back` breaks the tie, and it is a second fact
    // about a way rather than a fifth reading — see
    ...
    // The middle tiers compare the count, where they used to name `walked` and
    // `twice` by string. Those words are gone, so both rungs would have gone
    // quietly dead and this test would still have passed on a degraded ladder.
    //
    // `marks` is absent rather than nought on an unwalked way, so `<= 1` means
    // exactly one and needs no `wall` guard — unlike the same rung as a
    // *spell*, where a comparison answers nought for absence.
    let least = ways
        .into_iter()
...
/// marks` is answered by rather than a second opinion.
...
fn a_tester_cannot_farm_the_curve() {
    // `debug_spawn` makes reagents without work, which is why it must not pay:
    // a tester who buys concentration with a keystroke has a session that says
...
fn crossing_the_threshold_is_said_once_and_names_the_verb() {
    // §6 forbids a bare fact where a sentence would teach. It names `bind`,
...
fn what_was_earned_is_on_the_line_that_earned_it() {
    ...
    // laboratory's traffic. The run already says what it made, so what it
    // earned goes in that sentence — after it, not before the work.
    let mut sim = laboratory();
    run(&mut sim, &["kindle charcoal"]);
    grind(&mut sim);
...
///
/// No public way to hand the sim experience, deliberately (§11.5), so a test
/// reaches a tier the way a player does.
#[cfg(debug_assertions)]
...
///
/// `steps_1` was a marker for four phases until the menagerie gave it a reason:
/// that room cannot be automated at one instruction a tick.
#[cfg(debug_assertions)]
#[test]
...
///
/// The screen refuses it; this is the world agreeing, rather than trusting the
/// screen's arithmetic and having two answers to one rule (§19). Nothing on a
/// mastery line is taken — it is reached by doing.
#[cfg(debug_assertions)]
#[test]
