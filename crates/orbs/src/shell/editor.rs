...
//!   not harder.
//! - `orbs-tui` is a stub with nothing to diverge from.
...
impl Editor {
    ...
    #[must_use]
    pub(crate) fn open(name: &str, domain: &str, lines: &[String]) -> Self {
        Self {
            name: name.to_owned(),
    ...
    pub(crate) fn settle(&mut self, delta: f32) -> bool {
    ...
    pub(crate) const fn running_line(&self) -> Option<u64> {
        self.running_line
    }
    ...
    pub(crate) const fn complaint(&self) -> Option<&Complaint> {
        self.complaint.as_ref()
    }

    /// Where the caret is, as (row, column) in characters.
    pub(crate) const fn caret(&self) -> (usize, usize) {
    ...
    pub(crate) const fn scroll_to(&mut self, rows: usize) {
        if rows == 0 {
            return;
        }
    ...
    pub(crate) fn enter(&mut self) -> Option<Outcome> {
        match self.mode {
    ...
    #[must_use]
    pub(crate) fn visible(&self, row: usize, cells: u16) -> &str {
