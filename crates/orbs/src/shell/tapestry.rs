...
pub(crate) const WORDS: [(&str, Word); 4] = [
    ("ley", Word::Ley),
...
impl Tapestry {
    ...
    pub(crate) const fn track(&self) -> Track {
        self.track
    }
    ...
    /// The tower's total.
    pub(crate) const fn experience(&self) -> u64 {
        self.experience
    }

    /// Mastery's tiers, in order.
    pub(crate) fn mastery(&self) -> &[Vec<Node>] {
        &self.mastery
    }
    ...
    pub(crate) fn ley_line_columns(&self) -> Vec<Vec<Node>> {
        self.ley_line
            .iter()
    ...
    /// The node the cursor is on, if it has been placed.
    pub(crate) fn aimed(&self) -> Option<Node> {
    ...
    pub(crate) fn columns(&self) -> Vec<Vec<Node>> {
        match self.track {
            Track::LeyLine => self
                .ley_line
                .iter()
    ...
    pub(crate) fn step(&mut self, across: i8, down: i8) {
