...
pub(crate) fn split(area: Rect, board: Option<&Board>) -> Split {
    let nothing = Split {
        area: Rect::EMPTY,
        rest: area,
    };
    ...
    let tall = rows.saturating_add(2);

    // **Refused whole**, rather than clipped: every row of a sheet is a press,
    // and a sheet showing four of six presses is a sheet that has lost the two
    // a player was about to compare.
...
pub(crate) fn paint(painter: &mut Painter<'_>, at: Rect, board: &Board, prose: &Prose) {
    if at.is_empty() {
        return;
    }
