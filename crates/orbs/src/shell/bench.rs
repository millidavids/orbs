...
//!
//! It rides [`CrtSettings::on`](crate::crt::CrtSettings), read explicitly and
...
impl Bench {
    ...
    /// has to remember.
    pub(crate) fn burn(&self, lit: bool) -> Burn {
        if !self.enabled {
            return Burn {
                phase: 0.0,
                flare: 0.0,
    ...
    pub(crate) fn grind(&self, working: bool, spent: bool) -> Grind {
        Grind {
    ...
    ///
    /// **No pour, and that is a decision rather than an omission.** [`Self::load`]
    ...
    pub(crate) const fn stir(&self, motion: Motion, spent: bool, leavings: bool) -> Stir {
        Stir {
            phase: if self.enabled { self.phase } else { 0.0 },
            motion: if self.enabled {
                motion
    ...
    pub(crate) const fn observe_hearth(&mut self, lit: Option<bool>) {
    ...
    pub(crate) const fn observe_bowl(&mut self, charged: Option<bool>) {
    ...
    ///
    /// Separate from [`advance`] so it is testable without an `App`.
    ///
    ...
    pub(crate) fn tick(&mut self, seconds: f32) {
    ...
    pub(crate) const fn set_advance(&mut self, advance: f32) {
        self.advance = advance;
    }

    /// Put the pour part-way through, for a dump. See [`Self::set_flare`].
    pub(crate) fn set_load(&mut self, fraction: f32) {
    ...
    /// one; see `shell::dump`.
    pub(crate) fn set_flare(&mut self, fraction: f32) {
...
pub(crate) fn advance(
    ...
    panel: Res<super::input::Panel>,
    mut bench: ResMut<Bench>,
) {
    // No camera yet, or no CRT component on it: leave whatever `ORBS_FIRE` said.
    // A missing switch is not the same as a switch set to off, and defaulting to
    // "off" here would make the fire invisible for the frames before the camera
    // spawns rather than merely unlit.
    //
    ...
    //
    // **`permitted` is the cached read, not a fresh one.** `Bench::default`
    // documents reading the environment once "rather than polling it sixty times
    // a second forever" — and this line used to do exactly that polling, taking
    // the process environment lock and allocating a `String` per frame for a
    // value that cannot change during a run.
    let mut tubes = tubes.iter();
    if let Some(first) = tubes.next() {
        // Any tube off turns the motion off. Two switches disagreeing is not a
        // situation the game creates today, and the quieter reading is the only
        // safe one to pick if it ever does: §14's requirement is that a player
        // who asks for no motion gets none.
        let on = first.on && tubes.all(|other| other.on);
        let permitted = bench.permitted;
        bench.set_enabled(on && permitted);
    }
    // Where the world is between steps. **The same `Time<Fixed>` the sim runs
    // on** (`sim::plugin` installs it at 1 Hz), so it cannot disagree with the
    // sim about when a tick lands — which a second, self-counted copy would the
    // moment the clock hitched.
    bench.set_advance(world_tick.overstep_fraction());
    bench.observe_hearth(hearth(&panel));
    bench.observe_bowl(bowl(&panel));
    bench.tick(time.delta_secs());
}
...
/// `"mortar_and_pestle"`.
fn bowl(panel: &super::input::Panel) -> Option<bool> {
    panel
        .instruments
        .iter()
...
fn hearth(panel: &super::input::Panel) -> Option<bool> {
    panel
        .instruments
        .iter()
...
mod tests {
    ...
    fn a_second_camera_does_not_disconnect_the_motion_switch() {
        ...
        app.add_plugins(MinimalPlugins)
            .init_resource::<Bench>()
            .init_resource::<crate::shell::Panel>()
