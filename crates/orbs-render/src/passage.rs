...
//!
//! `attend forge` swaps the laboratory's instruments for the forge's lattice
//! between one frame and the next. §19 answered that for pane *geometry* with
//! [`crate::ScreenLayout::transition`]; this is the same for content.
//!
//! It is the glyphs moving, not a flash, a tube effect or a stinger — §19: *"no
//! part of this game flashes any more"*.
//!
...
//!
//! `progress` of `0.0` draws the kept cells exactly and `1.0` the live frame
//! exactly, so a settled crossing is indistinguishable from no crossing, which
//! keeps every `scripts/dumps.sh` baseline still.
//!
//! `pulse`'s exemption to the 3–30 Hz photosensitive band assumes nothing turns
//! over together, each step is small and the element stays small. A crossing
//! meets none of those, so it argues twice:
//!
//! - The shapes that erode — [`Passage::Wipe`] and [`Passage::Furl`] — argue per
//!   cell: coverage reverses once, decaying `▓ → ▒ → ░ → blank` on the way out
//!   and building back in. §19: *"a flash is a **pair** of opposing changes"*.
//! - The shape that moves — [`Passage::Gather`] — cannot: a glyph travelling
//!   past a cell lights it and darkens it again. Its property is a *field* one —
//!   everything it can light is inside an [`envelope`] shrinking monotonically
//!   to a point and growing back. `pulse`: *"whole-field modulation is the
//!   hazard; motion is not."*
//!
//! The per-cell rule was applied to all three at first and was too strict (§19).
//! How often a crossing may *start* is the shell's, a floor of one world tick.
...
///
/// Three, the rungs in the shade ramp. A wake of one is a hard edge that reads
/// as a rectangle drawn over the screen; a ramp-long one reads as the content
/// coming apart.
const WAKE: u16 = 3;

/// The shade ramp, most covered first.
///
/// CP437's three shading glyphs, so a crossing reads as *ASCII* rather than a
/// black rectangle sliding about. All three are in the repertoire — see
/// [`cp437`] — and `every_glyph_a_crossing_writes_is_drawable` holds it.
const RAMP: [char; 3] = ['▓', '▒', '░'];
...
///
/// The motion names the kind of change — a room, a tool, a re-reading (§19).
///
/// None of them translates a glyph: a cell showing a procession of source glyphs
/// as text slides past flickers between covered and blank, and this module rests
/// on a cell's coverage reversing *once*.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum Passage {
    /// A seam crosses the region; ahead of it the old screen, behind it nothing.
    ///
    /// A room change, the first crossing a player meets and the most legible of
    /// the three, so it must never read as a fault.
    #[default]
    Wipe,
    ...
    /// that moves a glyph rather than eroding it.
    ///
    /// A scale about the region's centre, read per destination as its inverse,
    /// which keeps it pure rather than a pass sorting overlapping writes.
    Gather,
}
...
impl Passage {
    ...
    ///
    /// Keeps [`cell_at`] from having to refuse a `Gather`: it took a whole
    /// [`Passage`] and `unreachable!`d on the one variant it cannot answer for,
    /// a panic on a *rendering* path. Split, the call cannot be written.
...
///
/// An erosion asks *"how far through is this cell"*, a convergence *"where does
/// this cell read from"*. This is the half [`cell_at`] can answer.
...
///
/// A plain `bool` would read as `true` at the call site, which
/// [`crate::ScreenRequest`]'s named fields exist to avoid. [`Passage::Gather`]
/// ignores it: both of its edges move at once.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum Toward {
    /// Out by the right edge, and back in from it.
    ///
    /// The instrument panel and the domain boards sit against that edge with
    /// the transcript to their left, so they leave by the side they are on
    /// rather than crossing the text on the way out.
    #[default]
    Right,
...
impl Crossing {
    ...
    /// so one number describes both the collapse and the growth back out.
    fn scale(self) -> f32 {
        if self.is_leaving() {
            1.0 - self.local()
        } else {
    ...
    ///
    /// A `NaN` compares false against everything, so
    /// [`is_leaving`](Self::is_leaving) on the raw field answered *arriving*
    /// while [`local`](Self::local) answered zero — an empty pane. Zero, so a
    /// bad number leaves the screen there was.
...
/// is moving. The rect travels with the cells, so a window resized mid-crossing
/// invalidates them rather than blitting last frame's cells at this frame's
/// coordinates.
///
...
impl Kept {
    ...
    ///
    /// Blank rather than `None`, which makes a crossing between two
    /// differently-shaped regions work: rows the old screen never had read as
    ...
    pub(crate) fn fill(&mut self, area: Rect, cells: impl Iterator<Item = Cell>) {
        self.area = area;
        // Cleared rather than reallocated: refilled every settled frame, and a
        // 120×45 grid of 8-byte cells is 43 KiB with no business reaching the
    ...
    /// Forget everything.
    ///
    /// Public where the refill is not: a player who asked for no motion should
    /// not pay 43 KiB for a screen nothing will cross out of.
    /// `Passing::set_enabled` calls it.
...
/// cell on the way in. Pure in position and progress — no RNG — which keeps
/// `ORBS_DUMP` reproducible and animation out of `orbs-sim`'s seeded streams.
pub(crate) fn cell_at(
    ...
    // Both erosions are the same wearing-away along a different ruler: how far
    // this cell is from the seam's start, how far the seam goes, and when this
    // cell's clock runs. One rule, so one safety argument rather than two.
    ...
    // The seam travels the span *plus* the wake, or the last cell's decay is cut
    // off mid-ramp and the half ends on a `▓` that never got to leave.
...
///
/// The inverse of the motion, which keeps this pure: forwards it is a *scatter*
/// a per-destination function cannot answer, backwards a sample — `d` shows
/// whatever was at `centre + (d - centre) / scale`.
///
/// Through [`tween::mix`], the crate's only `f32`-to-integer conversion, whose
/// guards land the edge cases free: at `scale` of zero the division is infinite,
/// every offset cell rounds outside the region, and the centre cell's `0 × ∞` is
/// a `NaN` `mix` answers with the centre.
...
///
/// Bounded before it is rounded. [`tween::mix`] clamps into `u16`'s range, right
/// for a coordinate and wrong for a *test*: `-∞` clamps to `0`, and `0` is
/// inside a region whose edge is the grid origin — it drew the centre column
/// three rows tall at the beat, and `examples/screens` showed it.
///
/// So the offset is judged in `f32` and `mix` is asked only for the rounding.
/// The `NaN` is the centre cell's `0 × ∞`, the one sample that always survives.
fn axis(centre: u16, at: u16, reach: f32, low: u16, high: u16) -> Option<u16> {
    ...
    // And again after rounding: anything in the last half-cell passes the test
    // above and comes back one past the end, where `Kept` holds the pane's own
    // border — a gather drew the bottom rule one row up, inside the interior,
    // for the first and last tenth of every `wander`. The `f32` test still
    // catches `-∞` clamping to `0`, so this is a second question, not a better
    // one.
...
///
/// Whole cells, a glyph not being drawable half in one. An even-sided region
/// takes the lower of its two middles, which only has to be *consistent* — the
/// same cell going out and coming back.
...
///
/// The safety property for a shape that moves rather than erodes: every cell it
/// can light is inside `centre ± half-extent × scale`, so the lit region is
/// *nested* — it shrinks monotonically to a point and grows back.
#[must_use]
...
///
/// The two halves measure from opposite edges, which makes a screen leave and
/// return the *same way*: going out toward the right the seam starts at the
/// left, and coming back the first content to appear is against the right edge.
/// Measured alike, a screen exits rightward and arrives from the left.
///
/// One arithmetic line, not a second copy of the rule with its comparisons
/// flipped.
...
///
/// A stagger rather than a slide: the two screens are the *same content
/// re-read*, so a translation would buy nothing.
///
/// The offset comes from [`crate::pulse::stagger`], an integer hash of position
/// rather than an RNG, so `ORBS_DUMP` stays reproducible.
...
///
/// A blank cell stays blank: coverage moves one way per half, and painting `▓`
/// over empty space would be a brightening on the way *out*.
///
/// Only the glyph changes, so a cell leaves in its own colour —
/// [`Depiction`](crate::Depiction) picks a ramp rather than a hue, and a flame's
...
///
/// The scale the safety property is stated on: blank, the three ramp rungs, then
/// anything else. Test-only — a claim *about* the ramp rather than something a
/// painter needs — but it lives beside the ramp so the two cannot drift apart.
#[cfg(test)]
...
mod tests {
    ...
    ///
    /// The two families answer different questions, so a property swept across
    /// all three asks each in its own terms. Every source here is the same
    /// glyph, so only *whether* a gather lands in the region matters.
    ...
    fn the_endpoints_are_identities() {
        // Everything else rests on this: a crossing that has not started and one
        // that has finished must be indistinguishable from no crossing, or a
        // settled `Passage` would move every `dumps.sh` baseline in the repo.
    ...
    fn the_midpoint_is_all_but_empty() {
        // The beat between the two screens, which makes the second half read as
        // an arrival rather than a cross-fade — illegible on a cell grid.
        //
        // All *but*: a `Gather` converges, so the one cell everything arrived at
        // is still lit at the beat.
        for passage in SHAPES {
            let standing = cells()
    ...
    fn an_eroding_cell_reverses_direction_once() {
        // The photosensitivity property for the shapes that erode. §19: "a flash
        // is a *pair* of opposing changes", so one reversal is one flash and the
        // shell can bound the rate with a single floor.
        //
        // `Gather` is held to `a_gather_never_lights_outside_its_envelope`
        // instead: the band is about the *field* modulating.
        //
    ...
    fn a_gather_never_samples_outside_its_region() {
        // Sources, not destinations — the hole
        // `a_gather_never_lights_outside_its_envelope` left. `Kept` holds the
        // whole grid, so one cell past the region is the pane border: a rounded
        // coordinate escaped by half a cell and drew the bottom rule inside.
        const STEPS: u16 = 400;
        for step in 0..=STEPS {
            let progress = f32::from(step) / f32::from(STEPS);
    ...
    fn a_gather_never_lights_outside_its_envelope() {
        // The field-level property the band is about. A cell may light and go
        // dark as the screen passes over it, but everything a gather can light
        // is inside a rectangle that shrinks to a point and grows back, so the
        // *lit area* never oscillates.
        //
        // Nested, too: each envelope of the leaving half is inside the one
        // before it, so the shrink is monotone rather than merely bounded.
        const STEPS: u16 = 240;
        ...
        for step in 0..=STEPS {
            ...
            // Nested *within* the leaving half. The two halves are mirror
            // images, so the envelope grows again across the beat and the
            // arriving half is the same claim reversed.
    ...
    fn a_gather_ends_on_the_middle_and_starts_there_again() {
        // The shape's whole claim, and why the middle must be the *same* cell in
        // both halves: leaving by one point and arriving from another reads as
        // two motions rather than one thing going and coming back.
        let (col, row) = centre(AREA);
    ...
    fn a_blank_cell_never_brightens_on_its_way_out() {
        // The rule `eroded` exists for: painting the ramp over empty space
        // raises coverage while the screen is *leaving*, which is the reversal
        // the safety argument turns on.
        for passage in SHAPES {
            for at in cells() {
                for step in 0..=40u16 {
    ...
    fn every_glyph_a_crossing_writes_is_drawable() {
        ...
        // fail silently — but a substituted ramp glyph draws `?` across the whole
        // wake, and the CP437 table is a thing edits reach into.
        for glyph in RAMP {
            assert!(
    ...
    fn an_upward_wipe_is_a_rightward_one_turned() {
        ...
        // really is the same motion turned rather than a second copy of the rule
        // with its comparisons transposed. A square region, so the axes compare.
    ...
    fn a_screen_returns_by_the_edge_it_left_by() {
        // Measured identically, a screen exits right and arrives from the left.
        //
        // Coverage rather than glyphs: near the beat the surviving edge is
        // inside its own wake, so the claim is *"more covered than the far
        // side"* rather than "still whole", a claim about the wake's length.
