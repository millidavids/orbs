...
An app password plus two HTTP calls. `release.yml` runs this once the tag and
GitHub Release exist. The body comes from the `### Description` section of the
version's changelog block, the same block Discord and the GitHub Release use.

Adapted from court_wizard's copy, minus the Steam link — there is no store page
yet and one that 404s is worse than three working links. Add it to `LINKS` when
there is one; a link is one tuple, not a special case.

Every post names the game in its title: these go out to a timeline, not to a
channel someone subscribed to.

Two AT Protocol traps, and the reason this is Python rather than a `curl` step:

  * A URL in the text is NOT a link — clickable links need an explicit `facet`.
    Since a facet can attach a URL to any span, the links ride on short anchor
    words, which matters against a 300-character limit.
  * Facet offsets are *byte* offsets. The changelog is full of em-dashes (3
    bytes each), so character indexing silently points a link at the wrong span.
    Offsets are recorded while building the string rather than searched for, so
    an anchor word occurring in the description cannot be matched by mistake.
...
#
# Source is here and not in court_wizard's set because this is a dev log for an
# unreleased game — the repository is what there is to look at. Add Steam when
# there is one and consider dropping Source then: four anchors plus separators
# is ~35 characters of the 300.
LINKS = [
    ("Website", GAME_SITE),
    ("Studio", STUDIO_SITE),
    ("Source", REPO_SITE),
...
def version_block(path: str, version: str) -> list[str]:
    ...
    By version rather than the top block: a manual dispatch can target an older
    release while the changelog on main has moved on.
    """
    try:
...
def description(block: list[str]) -> str:
    ...
    Falls back to the first bullet's lead-in, so a release missing the section
    still posts rather than failing after it is tagged and sent to Discord.
    """
    text: list[str] = []
    inside = False
    for line in block:
