...
editor, the weave, the stacks map. It is the category with the most
...
(`shell/sheet.rs`, `shell/editor.rs`), and the **stacks map**
(`shell/stacks.rs`, `shell/wandering.rs`, with the picture itself in
...
bound spell walks the stacks and the terminal frontend must draw them too, while
...
- **Take columns, not rows**, where you have the choice. The stacks map takes
...
- **Refuse rather than truncate.** Below a floor, draw a bordered "too small"
  ...
  misleading. `loom::MIN_ROWS`/`MIN_COLS` and `stacks::LEAST_BLOCK` are the
  precedents.
...
- **Transcribed geometry rots silently.** `stacks.rs`'s `BODIES` table holds
